...
std::vector<std::tuple<std::string, std::string, std::string>> EnumeratePCIDevices()
{
    ...
    while (SetupDiEnumDeviceInfo(deviceInfoSet, deviceIndex, &deviceInfoData))
    {
        ...
        if (SetupDiGetDeviceRegistryPropertyW(deviceInfoSet, &deviceInfoData, SPDRP_HARDWAREID, NULL, (PBYTE)hardwareId, sizeof(hardwareId), NULL) &&
            SetupDiGetDeviceRegistryPropertyW(deviceInfoSet, &deviceInfoData, SPDRP_DEVICEDESC, NULL, (PBYTE)deviceDesc, sizeof(deviceDesc), NULL))
        {
            auto vid_did = ExtractVidDid(hardwareId);
            std::wstring deviceDescWstr(deviceDesc);
            std::string deviceDescStr(deviceDescWstr.begin(), deviceDescWstr.end());
            if (deviceDescStr.empty()) {
                deviceDescStr = "Unknown Device";
            }
            devices.emplace_back(vid_did.first, vid_did.second, deviceDescStr);
        }
        deviceIndex++;
    }
